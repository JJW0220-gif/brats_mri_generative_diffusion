...
class VaeGanTrainer(Trainer):
    ...
    def __init__(
        ...
        self.seg_every_n_steps = max(1, int(seg_every_n_steps))
    ...
    def _iteration(
        ...
        seg_loss = torch.zeros(1, device=g_output.device, dtype=g_output.dtype)
        run_seg = engine.state.iteration % engine.seg_every_n_steps == 0
        if engine.segmentor is not None and engine.lambda_seg > 0.0 and run_seg:
            seg_loss = seg_losses.anatomical_segmentation_loss(
                segmentor=engine.segmentor,
                real_images=g_input,
                generated_images=g_output,
                wm_index=engine.seg_wm_index,
                gm_index=engine.seg_gm_index,
            )
            g_loss = g_loss + engine.lambda_seg * seg_loss
        g_loss.backward()
        ...
        return {
            ...
            "seg_loss": seg_loss.item() if isinstance(seg_loss, torch.Tensor) else float(seg_loss),
        }


class AnatomyAwareVaeGanTrainer(VaeGanTrainer):
    """Alias trainer for Stage-1 anatomy-supervised AE fine-tuning."""
